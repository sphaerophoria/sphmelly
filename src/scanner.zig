...
const ImageUpdater = struct {
    ...
    fn update(self: ImageUpdater, displayed_img_id: u32) !void {
        ...
        //for (0..expected_cpu.buf.len / 2) |i| {
        //    const tmp = expected_cpu.buf[i];
        //    expected_cpu.buf[i] = expected_cpu.buf[expected_cpu.buf.len - 1 - i];
        //    expected_cpu.buf[expected_cpu.buf.len - 1 - i] = tmp;
        //}
...
fn buildUi(allocators: anytype, gui_alloc: gui.GuiAlloc, displayed_img_info: *usize, display_mode: *DisplayMode, infos: []ImgInfo) !Widgets {
    ...
    for (infos, 0..) |info, info_idx| {
        ...
        // FIXME: HACK someone isn't padding correctly
        try overview_layout.pushWidget(
            try widget_factory.makeInteractable(
    ...
    const one_of = try widget_factory.makeOneOf(
        ...
        &.{
            try widget_factory.makeScrollView(overview_layout.asWidget()),
            layout.asWidget(),
        },
    );
...
pub fn main() !void {
    ...
    const expected_cpu = try math_executor.toCpu(cl_alloc.heap(), &cl_alloc, bars.bars);

    var img_infos = std.ArrayList(ImgInfo).initBuffer(
        try allocators.root.arena().alloc(ImgInfo, stage1_config.data.batch_size),
    );

    for (0..stage1_config.data.batch_size) |img_id| {
        const output_len = 95;
        const output_start = img_id * output_len;

        var correct_bars: usize = 0;
        var correct_flipped_bars: usize = 0;
        for (0..output_len) |i| {
            const predicted_true = predicted_codes_cpu[output_start + i] > 0.5;
            const predicted_flipped_true = predicted_codes_flipped_cpu[output_start + i] > 0.5;
            const expected_true = expected_cpu[output_start + i] > 0.5;

            if (predicted_true == expected_true) correct_bars += 1;
            if (predicted_flipped_true == expected_true) correct_flipped_bars += 1;
        }

        const max_correct = @max(correct_bars, correct_flipped_bars);
        try img_infos.appendBounded(.{
            // FIXME: HACK someone isn't padding correctly
            .name = try std.fmt.allocPrint(allocators.root.arena(), " img {d:0>3}", .{img_id}),
    ...
    std.mem.sort(ImgInfo, img_infos.items, {}, struct {
        fn f(_: void, a: ImgInfo, b: ImgInfo) bool {
    ...
    var widgets = try buildUi(&allocators, gui_alloc, &displayed_img_info, &display_mode, img_infos.items);
    ...
    try updater.update(img_infos.items[displayed_img_info].id);
