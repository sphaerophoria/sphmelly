...
pub fn makeBars(self: BarcodeGen, params: MakeBarsParams) !Bars {
    ...
    if (params.extract_params) |extract_params| {
        ...
        out = try self.math_executor.reshape(params.cl_alloc, out, &.{ params.output_size, params.output_size, num_barcodes });
    } else {
        if (params.output_size != self.background_size) return error.InvalidResize;
    }
...
fn extractFromBoxes(self: BarcodeGen, cl_alloc: *cl.Alloc, box_labels: math.Executor.Tensor, imgs: math.Executor.Tensor, extract_params: ExtractParams, rand_source: *math.RandSource, target_size: u32) !math.Executor.Tensor {
    ...
    return try self.math_executor.downsampleBox(
        cl_alloc,
