        ...
        switch (pause) {
            .unpaused => {
                ...
                if (config.train_target == .bbox) {
                    try barcode_gen.healBboxLabels(&cl_alloc, train_input.expected, tracing_executor.getClTensor(results[results.len - 1].buf), config.data.confidence_metric, config.disable_bbox_loss_if_out_of_frame);
                }
                    ...
                    switch (config.train_target) {
                        .bbox => {
                            ...
                            if (iter % config.val_freq == 0) {
                                ...
                                try barcode_gen.healBboxLabels(&cl_alloc, validation_set.expected, val_results, config.data.confidence_metric, config.disable_bbox_loss_if_out_of_frame);
